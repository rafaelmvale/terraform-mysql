...
resource "azurerm_resource_group" "example-mysql" {
    name = "example-mysql"
    location = "eastus"

    tags = {
        "Enviroment" = "Example Test"
    }
...
resource "azurerm_virtual_network" "virtualnetmysql" {
    ...
    location            = "eastus"
    resource_group_name = azurerm_resource_group.example-mysql.name

    depends_on = [ azurerm_resource_group.example-mysql ]
}
